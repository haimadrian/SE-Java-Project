...
import org.spa.view.table.editor.ButtonColumn;
import org.spa.view.table.editor.CountCellEditor;
import org.spa.view.table.editor.TextCellEditor;
import org.spa.view.table.renderer.SpinnerCellRenderer;
import org.spa.view.table.renderer.StretchedImageCellRenderer;
   ...
   private static class Constants {
      public static final ButtonColumn CART_BUTTON = new ButtonColumn(table -> SwingUtilities.invokeLater(() -> {
         ...
         if (selection != null) {
            ...
            try {
               Item shoppingCartItem = shoppingCart.getItems().stream().filter(item -> item.getId().equals(selectedItemId)).findFirst().orElse(null);
